...
def get_log_p_and_kl_enumerate(true_generative_model, generative_model, inference_network, obs,
                               num_particles=None, reweighted_kl=False):
    """
    Args:
    ...
    latent_expanded = latent[:, None, :].repeat(1, batch_size, 1)  # [num_all_zs, batch_size, num_data]
    ...
    return log_p, log_p_true, kl_qp, kl_pq, kl_qp_true, kl_pq_true, reweighted_kl_qp, reweighted_kl_qp_true
...
def eval_gen_inf(true_generative_model, generative_model, inference_network, memory, data_loader,
    ...
    (log_p_total, log_p_true_total,
     ...
     reweighted_kl_qp_total, reweighted_kl_qp_true_total) = 0, 0, 0, 0, 0, 0, 0, 0
    num_data = data_loader.dataset.shape[0]
    for obs in iter(data_loader):
        (log_p, log_p_true, kl_qp, kl_pq, kl_qp_true, kl_pq_true,
         reweighted_kl_qp, reweighted_kl_qp_true) = get_log_p_and_kl_enumerate(
            true_generative_model, generative_model, inference_network, obs,
            num_particles, reweighted_kl)
    ...
    if reweighted_kl:
        return (log_p_total, log_p_true_total,
                kl_qp_total, kl_pq_total, kl_qp_true_total, kl_pq_true_total,
                kl_memory_p, kl_memory_p_true, reweighted_kl_qp_total, reweighted_kl_qp_true_total)
    else:
        return (log_p_total, log_p_true_total,
                kl_qp_total, kl_pq_total, kl_qp_true_total, kl_pq_true_total,
